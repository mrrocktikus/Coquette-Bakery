...
const InventoryList = ({ t, inventory, onDelete, onUpdate }: Props) => {
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1 md:w-64">
            ...
            <Input 
              ...
              className="pl-10 rounded-xl border-2 border-rose-200"
            />
          </div>
          <Button 
            variant="outline" 
            onClick={() => setShowFilters(!showFilters)}
            className="rounded-xl border-2 border-rose-200 text-rose-500"
          >
        ...
        <div className="overflow-x-auto">
          <Table>
            <TableHeader className="bg-rose-50/50">
              <TableRow>
                <TableHead className="font-bold text-rose-700">{t('name')}</TableHead>
                <TableHead className="font-bold text-rose-700">{t('type')}</TableHead>
                <TableHead className="font-bold text-rose-700">{t('category')}</TableHead>
                <TableHead className="font-bold text-rose-700">{t('qty')}</TableHead>
                <TableHead className="font-bold text-rose-700">{t('status')}</TableHead>
                <TableHead className="font-bold text-rose-700 text-right">{t('actions')}</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item) => (
                  <TableRow key={item.id} className="hover:bg-rose-50/30 transition-colors">
